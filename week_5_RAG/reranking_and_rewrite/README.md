...
├── retrieval_v2.py        # Шаг 2 — broad retrieval (top-10) + HARD cosine threshold + cross-encoder rerank (top-3)
├── agent_v2.py            # Шаг 3 — ask_agent_v2(question, mode='basic'|'advanced')
├── generation_v3.py       # Этап 4 — структурированный ответ (Ответ/Цитаты/Источники)
├── evaluate_v2.py         # Шаг 4 — автотест: Basic vs Advanced RAG → report_v2.md
├── report_v2.md           # Отчёт этапа 3
├── eval_questions_v3.json # Этап 4 — 8 вопросов из книги + 2 вне книги (проверка "Не знаю")
├── evaluate_v3.py         # Этап 4 — автотест: структура ответа + порог отсечения → report_v3.md
├── report_v3.md           # Отчёт этапа 4
└── main_rag.py            # Этап 4 — демо с подробными логами порога и структуры ответа
```
...
## Этап 4 — Структурированные ответы + жёсткий порог отсечения ("Не знаю")

Раздел выше объяснял, почему cosine-threshold заменили на cross-encoder **для выбора лучших чанков** — в этой роли у порога были слепые зоны. Здесь порог возвращается, но **в другой роли**: не выбирать лучшие чанки (это по-прежнему делает cross-encoder), а решить, стоит ли вообще пытаться отвечать. Это два разных вопроса: "какие из найденных чанков лучше" (cross-encoder) vs "достаточно ли вообще релевантен весь top-10, чтобы не галлюцинировать" (hard threshold).

### Структурированный ответ (`generation_v3.py`)

`generate_structured_answer(question, chunks, language=None)` — системный промпт (на английском) заставляет DeepSeek вернуть строго три Markdown-блока в фиксированном порядке:

1. **`## Answer`** — краткий ответ (1-3 предложения) только по контексту.
2. **`## Quotes Used`** — дословные выдержки из контекста (в кавычках, каждая с новой строки), на которых основан ответ. Модели явно запрещено перефразировать цитату.
3. **`## Sources`** — список `source: <файл эпаба> | section: <глава> | chunk_id: <id>` по каждому использованному чанку.

`build_context_v3()` подаёт модели каждый чанк с явным тегом `[chunk_id: ... | source: ... | section: ...]` перед текстом — иначе модели неоткуда взять точные chunk_id для блока "Источники".

### Жёсткий порог отсечения (`retrieval_v2.py`, `SIMILARITY_THRESHOLD = 0.60`)

`retrieve_chunks_advanced()` теперь после broad retrieval (top-10, cosine) сразу проверяет: `max(score for score in top-10) >= 0.60`?

- **Нет** → возвращает `{"kept": [], "threshold_passed": False, "max_score": ...}` и обрывает пайплайн **до** cross-encoder rerank и **до** любого обращения к DeepSeek.
- **Да** → идёт дальше в `rerank_with_cross_encoder()` как раньше.

`agent_v2.py` проверяет `threshold_passed` раньше всего остального в режиме `advanced`: если порог не пройден — API DeepSeek не вызывается вообще, пользователю сразу возвращается константа `HARD_REFUSAL_ANSWER` (на английском, как и все вопросы/ответы в этом наборе):

> "Unfortunately, 'Alice's Adventures in Wonderland' does not contain enough facts to answer this question. Please rephrase or clarify your request."

### Автотест на 10 вопросах (`eval_questions_v3.json`, `evaluate_v3.py` → `report_v3.md`)

Все вопросы и ответы — на английском. 8 вопросов из книги + 2 заведомо не из книги ("What year did Harry Potter meet Alice?", "What make of car did the March Hare drive?"). Для каждого вопроса скрипт проверяет: есть ли блок `## Sources`, есть ли блок `## Quotes Used`, и сработал ли режим отказа именно там, где должен (не сработал на вопросах из книги, сработал на вопросах не из книги).

**Реальный прогон (см. `report_v3.md`): 10/10.** Оба вопроса не из книги корректно получили отказ без обращения к DeepSeek (max cosine 0.5478 и 0.5780, оба ниже порога 0.60). Все 8 вопросов из книги прошли порог и получили структурированный ответ с цитатами и источниками. В отдельном прогоне до этой правки один пограничный вопрос (Cheshire Cat, max cosine 0.5920) не проходил порог 0.60 — честная демонстрация цены жёсткого порога: он снижает recall в обмен на гарантию отсутствия галлюцинаций при слабом контексте. Порог `0.60` — настраиваемый компромисс, а не абсолютная истина.

### `main_rag.py`

Демо-скрипт (аналог `request_to_RAG/main_rag.py`, но на пайплайне этапа 4): для каждого вопроса печатает, пройден ли порог релевантности (с точным cosine-числом), вызывался ли DeepSeek вообще, и если да — структурированный ответ модели целиком плюс метаданные всех источников для ручной проверки, что цитаты не выдуманы.
